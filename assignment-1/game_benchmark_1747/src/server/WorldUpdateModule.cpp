...
void WorldUpdateModule::run()
{
  ...
  Uint32 wui, rui;
	...
	Player *p;
  // NOTE: sd is an instance of ServerData
  ...
  int loop_count = 0;
    ...
    while( (m = comm->receive( timeout, t_id )) != NULL )
    {
        ...
        switch ( type )
        {
            ...
            default: 
              printf("[WARNING] Unknown message (%d) received from %u.%u.%u.%u:%d\n", m->getType(), 
    ...
    */ 
    ...
    {
    	ms = new MessageWithSerializator( MESSAGE_SC_REGULAR_UPDATE, t_id, p->address );
      assert(ms);
